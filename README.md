# Build a Photo Sharing App with Next.js and AWS Amplify
...
일단, [Create Next App](https://nextjs.org/docs/api-reference/create-next-app) 을 이용하여 새로운 프로젝트를 생성할것입니다. 그다음은 [Amplify CLI](https://github.com/aws-amplify/amplify-cli) 를 이용하여 AWS 위에 Cloud 환경을 만들고 설정하려 합니다. 그 뒤에는 [Amplify JS Libraries](https://github.com/aws-amplify/amplify-js) 를 이용하여 우리가 만든 next.js 앱을 Amplify CLI 로 이용하여 생성한 api 와 연결해보려 합니다.

이 앱은 기본 CRUD, List 와 실시간 업데이트 기능이 들어갑니다. Facebook, Twitter, Instagram 과 같은 앱들을 보면, 목록에서 선택하면, 싱글 아이템을 볼수 있는 뷰로 들어갑니다. 이 앱도 그와 비슷하게, 사진과 이름, 위치, 상세정보가 들어있는 포스트의 목록들이 나오는 뷰와, 본인의 포스트 목록만 보여주는 뷰를 만들어보려 합니다.
...
$ yarn add emotion uuid react-router-dom
```
...
우선 본 프로젝트를 위한 git repository를 하나 만들어주세요. (https://github.com/new)
